...
const MapView = forwardRef(
    (
        ...
        useEffect(() => {
            ...
            // 현재 위치 마커 (파란색 원)
            const currentPositionMarker = new window.kakao.maps.CustomOverlay({
            ...
            // 점포 마커 생성
            console.log(`${markers.length}개의 마커 생성 시작`);
            markers.forEach((markerData, index) => {
                console.log(`마커 ${index + 1} 생성 시도:`, markerData);

                if (!markerData.latitude || !markerData.longitude) {
                    console.warn(
                        `마커 ${index + 1}에 위치 정보가 없습니다:`,
                        markerData
                    );
                    return;
                }
                    ...
                    const infowindow = new window.kakao.maps.InfoWindow({
                        content: `
                            <div class="custom-overlay">
                                <strong>${markerData.title || '점포'}</strong>
            ...
            console.log('마커 생성 완료, 총', markersRef.current.length, '개');
        }, [isMapLoaded, markers, center, onMarkerClick]);
